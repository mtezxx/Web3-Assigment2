...
function handleBotDecision(e: MessageEvent<BotDecision>) {
  ...
  if (msg.decision === 'sayUno') safe(() => store.sayUno())
...
onBeforeUnmount(() => {
  ...
  // restore originals to avoid stacking wrappers
...
<template>
  <div class="p-4" v-if="round">
    ...
    <section class="mt-3">
      ...
      <p class="mt-2">
        Top card:
        <span>{{ label(round.discardPile().top()) }}</span>
      </p>
    </section>
